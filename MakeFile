...
ALL_SRCS  := $(filter-out $(COMMON_SRC),$(wildcard $(SRC_DIR)/*.c))
...
DEPS      := $(CORE_BINS:=.d) $(UV_BINS:=.d) $(COMMON_OBJ:.o=.d)
...
$(BUILD_DIR)/uv_%: $(SRC_DIR)/uv_%.c $(COMMON_OBJ) | $(BUILD_DIR)
	$(CC) $(CFLAGS) $< $(COMMON_OBJ) $(LDFLAGS) $(UV_LIBS) -o $@
