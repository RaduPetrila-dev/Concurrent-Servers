...
static int line_value(const char *body, const char *prefix, unsigned long long *out)
{
...
static void test_counters_and_gauges(void)
{
    ...
    assert(line_value(body, "server_connections_accepted_total{model=\"epoll\"}", &value) == 0);
    assert(value == 7);
    assert(line_value(body, "server_connections_refused_total{model=\"epoll\"}", &value) == 0);
    assert(value == 1);
    assert(line_value(body, "server_connections_active{model=\"epoll\"}", &value) == 0);
    assert(value == 5);
    assert(line_value(body, "server_pool_queue_depth{model=\"epoll\"}", &value) == 0);
    assert(value == 64);
    assert(line_value(body, "server_pool_workers_busy{model=\"epoll\"}", &value) == 0);
    assert(value == 2);
}
...
static void test_histogram_boundaries_are_inclusive(void)
{
    ...
    assert(value == 3);
    assert(line_value(body, "server_request_duration_seconds_count", &value) == 0);
    assert(value == 3);
    assert(strstr(body, "server_request_duration_seconds_sum{model=\"threadpool\"} 0.060040001") !=
           NULL);
}
...
static void test_concurrent_updates_do_not_lose_counts(void)
{
    ...
    assert(line_value(body, "server_connections_accepted_total{model=\"threadpool\"}", &value) == 0);
    assert(value == (unsigned long long)THREADS * PER_THREAD);
    assert(line_value(body, "server_connections_active{model=\"threadpool\"}", &value) == 0);
    assert(value == 0);
    assert(line_value(body, "server_request_duration_seconds_count", &value) == 0);
    assert(value == (unsigned long long)THREADS * PER_THREAD);
}
