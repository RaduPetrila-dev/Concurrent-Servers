...
static const uint64_t k_bucket_upper_ns[BUCKET_COUNT] = {
    20000, 50000, 100000, 250000, 500000, 1000000, 2500000, 5000000, 10000000, 50000000,
};
...
{
    atomic_fetch_add_explicit(&g.latency_count[bucket_index(duration_ns)], 1, memory_order_relaxed);
    atomic_fetch_add_explicit(&g.latency_sum_ns, duration_ns, memory_order_relaxed);
}
...
static void render_counter(char *out, size_t cap, size_t *len, const char *name, const char *help,
                           uint64_t value)
{
    appendf(out, cap, len, "# HELP %s %s\n", name, help);
    appendf(out, cap, len, "# TYPE %s counter\n", name);
    appendf(out, cap, len, "%s{model=\"%s\"} %" PRIu64 "\n", name, model_label(), value);
}

static void render_gauge(char *out, size_t cap, size_t *len, const char *name, const char *help,
                         int64_t value)
{
    appendf(out, cap, len, "# HELP %s %s\n", name, help);
...
static void render_latency(char *out, size_t cap, size_t *len)
{
    ...
    for (size_t i = 0; i < BUCKET_COUNT; i++) {
        cumulative += atomic_load_explicit(&g.latency_count[i], memory_order_relaxed);
        appendf(out, cap, len, "%s_bucket{model=\"%s\",le=\"%s\"} %" PRIu64 "\n", name,
                model_label(), k_bucket_le[i], cumulative);
    }

    cumulative += atomic_load_explicit(&g.latency_count[BUCKET_COUNT], memory_order_relaxed);
    ...
    appendf(out, cap, len, "%s_sum{model=\"%s\"} %" PRIu64 ".%09" PRIu64 "\n", name,
            model_label(), sum_ns / 1000000000ULL, sum_ns % 1000000000ULL);
...
size_t metrics_render(char *out, size_t cap)
{
    ...
    render_gauge(out, cap, &len, "server_connections_active", "Connections currently open.",
                 atomic_load_explicit(&g.active, memory_order_relaxed));
    render_gauge(out, cap, &len, "server_pool_queue_depth",
                 "Work items waiting in the thread pool queue.",
                 (int64_t)atomic_load_explicit(&g.queue_depth, memory_order_relaxed));
    render_gauge(out, cap, &len, "server_pool_workers_busy", "Worker threads serving a request.",
